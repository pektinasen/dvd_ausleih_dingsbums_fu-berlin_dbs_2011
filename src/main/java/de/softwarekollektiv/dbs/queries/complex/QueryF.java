...
class QueryF implements MenuItem {
	...
	public boolean run() throws Exception {
		...
		for (String[] pair : pairs) {
			...
			out.println("Shortest path between " + pair[0] + " and " + pair[1]
					+ " is: " + actIdsToNames(shortestPath(startId, endId)));
		}
	...
	private List<String> actIdsToNames(List<Integer> shortestPathAsList)
			throws SQLException {
		PreparedStatement actIdToNameStmt = dbcon.getConnection()
				.prepareStatement("SELECT name FROM actors WHERE act_id = ?");
